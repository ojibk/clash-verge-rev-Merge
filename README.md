...
* *  · 在本文件中定义固定组名 [节点选择]，rules 段硬编码的代理出口指向此名称。该组由本文件保证必然存在，切换任意订阅均不会因找不到组名而触发启动报错。
...
proxy-groups:
  - name: 节点选择     * 🛡️ 策略组锚点桩 | rules 规则段唯一关联策略组出口。 💡 硬编码固定策略组名，以确保此覆写配置文件的普适性。
    ...
    include-all: true * ⚙️ Mihomo 内核支持字段：自动继承订阅内所有 outbound proxies 及 proxy sets
                      *    （proxy groups 及嵌套策略组均不在继承范围内），切换订阅后节点列表自动同步，无需手动维护 proxies 数组。
  ...
  - name: 非港自动选择 * 声明策略组名称，优选除香港外的低延迟节点
    type: url-test    * 策略组类型,自动延迟测试优选组。⚠️ 风险：IP 频繁变动（可能触发网络服务风控）；无谓的后台探测流量消耗
    url: http://www.gstatic.com/generate_204 * 延迟测试链接
    * url: http://cp.cloudflare.com/generate_204 * 备用延迟测试链接
    interval: 1800    * 健康检查间隔，如不为 0 则启用定时测试，默认值 300，单位：秒。为避免 IP 频繁变动触发风控，设为半小时（不一定能规避风控）
    ...
    filter: '^(?!.*(香港|HK|Hong\s*Kong|🇭🇰|🳪)).*$' * 使用单引号包裹正则，利用负向先行断言过滤包含 香港、HK、HongKong、Hong Kong、🇭🇰、🳪 的节点
...
rule-providers:
  ...
  *                     *    备用直链：https://github.com/peiyingyao/Rule-for-OCD/raw/refs/heads/master/rule/Clash/EasyPrivacy/EasyPrivacy_OCD_Domain.mrs
  ...
  hk_restricted: * 🇭🇰 港区受限网络服务专项规则集（含 OpenAI、Claude、Gemini 精准子域、TikTok 全球资源）
    type: http        * ℹ️ 类型：远程 HTTP 资源。
    behavior: domain  * ℹ️ 行为：域名模式，MRS 直接内存映射已构建的后缀匹配树（Domain Trie）。
    format: mrs       * ℹ️ 格式：MRS 二进制格式
    path: "./ruleset/providers/hk_restricted.mrs" * 存储路径。规则集文件命名与规则集声明键值解耦
    interval: 604800  * ⏱️ 更新间隔：604800 秒 = 7 天（内核运行时按该周期后台定时拉取；重启后亦检测缓存是否过期并重新拉取，不保证实时同步）。
    url: "https://testingcf.jsdelivr.net/gh/ojibk/rules@main/hk_restricted.mrs"
                      *    备用直链：https://raw.githubusercontent.com/ojibk/rules/main/hk_restricted.mrs
...
rules:
  ...
  * ▌域名分流 — cn 规则集在前确保国内域名优先命中
  - RULE-SET,hk_restricted,非港自动选择    * 🇭🇰 专属港区受限网络服务高可用路由规则 → 走非港节点代理，置于分流集的顶层是避免被代理规则抢先命中而误走其他节点。
...
* *   proxy sets（proxy-providers 声明的节点集合）注入当前策略组
