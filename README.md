...
* * Clash-Merge 全局扩展覆写配置 · 锚点组与规则链（智能路由锁区受限网络服务） v260602
* * --------------------------------------------------------------------------------------
* * 存储路径参考：
...
* *  🇭🇰 智能路由锁区受限网络服务 → 走非香港节点，应用场景示例：指定某些对香港 IP 极为敏感的 AI、流媒体等网络服务（含 OpenAI、Claude、Gemini、TikTok）的路由策略。
* *
* *  职责分工：
...
geodata-mode: true              * ⚙️ GeoIP 数据库格式：true = dat 格式（数据文件由 MetaCubeX 自维护），false 或缺省（未声明）= mmdb 格式（Mihomo 默认）。
                                ...
                                *    缩小两者"规则集归类为国内、内置 GeoIP 库认定为境外"的归类不一致风险。切换理由仅为统一归属判定基准（无更新频率收益）。
...
geox-url:                       * ⚙️ geodata 下载地址。未声明时 Mihomo 回退至内置默认地址（指向 GitHub Release 直链，国内大概率无法访问）。
  ...
  * ℹ️ ASN（自治系统号）数据库用于 Mihomo 的 GEOASN, 规则匹配指令，当前配置未声明 GEOASN 规则，该字段仅为预留。
...
* * ⚠️ sniffer 配置是扩展脚本（Script.js）注入的 UDP/QUIC 域名类规则重要前提条件之一，
* * 禁用 sniffer 将导致这些规则在纯 IP 直连路径（应用绕过 DNS、直接以 IP 建立 UDP/QUIC 连接）下失效。
* * force-dns-mapping: true：使 Script.js 注入的 DOMAIN 类 AND 多条件组合规则能通过 DNS 映射缓存识别域名。
...
sniffer:
  ...
  force-dns-mapping: true       * ⚙️ 将嗅探域名写入 DNS 映射缓存（若未存在），使规则匹配阶段可用嗅探到的域名进行查询，而非仅凭连接 IP，显著提升域名规则集的命中率。
  ...
  sniff:
    TLS:
      ports: [443, 8443]        * ℹ️ 标准 HTTPS 端口 + 常见 HTTPS 备用端口。
    HTTP:
      ports: [80, 8080, 8880, 2052, 2082, 2086, 2095]
                                * ℹ️ 标准 HTTP 端口 + Cloudflare 常用 HTTP 备用端口组合。
    QUIC:
...
* *  · 在本文件中定义固定组名 [🎛️ 节点选择]，rules 段硬编码的代理出口指向此名称。该组由本文件保证必然存在，切换任意订阅均不会因找不到组名而触发启动报错。
...
* *
* * ⚠️ 【跨客户端兼容性警告】
* *  本配置依赖 include-all 字段自动将订阅裸节点填充至 [🎛️ 节点选择] 组。
* *  该字段由 Mihomo 内核实现，CVR 完整支持；但在 Stash / ClashX / OpenClash 等客户端中该字段被静默跳过，[🎛️ 节点选择] 将成为空组，导致全局网络中断。
* *  非 CVR 用户须启用下方 proxies 行，将占位符替换为实际节点名，例如 proxies: [节点A, 节点B]
* *  ⚠️ 继承的节点须为独立可用的底层节点；若订阅仅提供 relay / url-test / fallback 等策略组， include-all 无法继承策略组内容，[🎛️ 节点选择] 将成为空组。
* *
* * 【已知代价】
* *  proxy-groups 为 Array 类型（有序列表），声明即触发全量替换，订阅原有所有策略组被丢弃。
* *  代理面板仅保留 [🎛️ 节点选择] 一个出口，由用户手动切换节点。订阅原有精细策略组（自动选择 / ChatGPT / Netflix 等）将完全消失。
* *  若需同时保留订阅策略组与锚点组，须注释掉本文件的 proxy-groups 段，改由脚本主导。

proxy-groups:
  - name: 🎛️ 节点选择  * 🛡️ 策略组锚点桩 | rules 规则段唯一关联策略组出口。 💡 硬编码固定策略组名，以确保此覆写配置文件的普适性。
                      ...
                      * ⚠️ 架构风险：若剥离此静态桩，切换订阅时将因组名不一致丧失“语义稳定性”，导致内核配置加载报错。
    type: select      * ℹ️ 类型：手动选择，由用户在 UI 代理面板点击切换当前生效节点。
    include-all: true * ⚙️ Mihomo 内核支持字段：自动继承订阅内所有 outbound proxies 及 proxy sets（proxy groups 及嵌套策略组均不在继承范围内），
                      *    切换订阅后节点列表自动同步，无需手动维护 proxies 数组。
                      *    在 Stash / ClashX 等客户端中该字段不受支持，静默跳过（即该策略组不包含任何节点，成为空组）。
                      * ⚠️ 隐式前提：订阅须包含 outbound proxies（底层代理节点）。
                      *    以下两种情况均会导致 include-all 未能继承任何节点，[🎛️ 节点选择] 成为空组：
                      *    · 机场仅提供策略分组，不对外提供裸节点。
                      *    · 订阅仅提供 relay / url-test / fallback 等策略组，且未将原始节点对外提供（界面看似有节点，实则不可被继承）。
                      * ⚠️ 外部系统风险：若订阅源返回空节点（订阅失效 / 机场清空节点），本配置将整体不可用——Fail-Fast 会将外部错误显式暴露为全局连接失败。
                      *    遇此情况须检查订阅源，或改用硬编码订阅组名方案。
                      *    ℹ️ 空组时触发 Fail-Fast（异常立即暴露为连接失败，不静默降级，见术语字典）。

    * ⚠️ select 手动选择 嵌套 url-test 自动选择 架构的代价与状态机冗余（都 include-all: true 时）：
    * 内存占用重复：内核在启动阶段需要在内存中实例化两套完全重叠的代理节点指针数组，造成内存重复占用，节点数量庞大时尤甚。
    * 后台探测开销：url-test 策略组不是静态的，会按 interval 周期在后台拉起独立的高并发延迟探测协程，不断对 include-all 抓取的所有节点发起 HTTP/TCP 探针用于选路。
    proxies:
      - 🤖 自动选择 * 显式将自动组作为顶层的一个可选分支注入
    ...
    * proxies: [🔴 请手动填入节点]    * ⚠️ 非 CVR 客户端必须手动填入节点名称，例如：proxies: [节点A, 节点B]，应替换 []。
                      ...
                      *    此占位条目指向不存在的节点名称，等效于空组，所有通过该策略组出站的连接将失败。
  * ⚠️ 注意：在 type: url-test 策略组中手动点选节点视为锁定（固定所选节点），自动切换机制暂时失效直到点击延迟测试后解除；也可清理 cache.db 或重启 CVR。
  - name: 🔀 港外优选  * 声明策略组名称，优选除香港外的低延迟节点。🇭🇰 专属港区网络服务受限场景：对香港 IP 敏感的 AI、流媒体等（如 chatGPT、Claude、Gemini、TikTok）。
    type: url-test    * 策略组类型,自动延迟测试优选组。⚠️ 已知风险：IP 频繁变动（可能触发网络服务风控）；无谓的后台探测流量消耗
    interval: 1800    * 健康检查间隔，如不为 0 则启用定时测试，默认值 300，单位：秒。为避免 IP 频繁变动触发风控，暂设半小时（不一定能规避风控），可根据实际情况动态调整
    tolerance: 50     * 节点切换容差，单位：毫秒。只有当新旧节点的差值大于 50ms 内核才会触发切换。
    include-all: true * 引入所有出站代理以及代理集合，顺序将按照名称排序。自动继承订阅源中的全量节点
    lazy: true        * true懒惰状态，默认为true,未选择到当前策略组时，不进行测试
    ...
    * url: http://cp.cloudflare.com/generate_204 * 备用延迟测试链接
    ...
    filter: '^(?:(?!香港|HK|Hong\s*Kong|🇭🇰|🳪).)*$' * 正则表达式过滤，反引号包裹。正向匹配过滤 香港、HK、HongKong、Hong Kong、🇭🇰、🳪 的节点
    * filter: '^(?!.*(香港|HK|Hong\s*Kong|🇭🇰|🳪)).*$' * 正则表达式过滤，需引号包裹。利用负向先行断言过滤包含 香港、HK、HongKong、Hong Kong、🇭🇰、🳪 的节点

  - name: 🤖 自动选择 * 声明策略组名称
    type: url-test    * 策略组类型,自动延迟测试优选组。⚠️ 已知风险：IP 频繁变动（可能触发网络服务风控）；无谓的后台探测流量消耗
    interval: 300     * 健康检查间隔，如不为 0 则启用定时测试，默认值 300，单位：秒。可根据实际情况动态调整
    tolerance: 50     * 节点切换容差，单位：毫秒。只有当新旧节点的差值大于 50ms 内核才会触发切换。
    include-all: true * 引入所有出站代理以及代理集合，顺序将按照名称排序。自动继承订阅源中的全量节点
    lazy: true        * true懒惰状态，默认为true,未选择到当前策略组时，不进行测试
    url: http://www.gstatic.com/generate_204 * 延迟测试链接
    * url: http://cp.cloudflare.com/generate_204 * 备用延迟测试链接
    * expected-status: 204 * 当健康检查返回状态码与期望值不符时，认为节点不可用。为注释状态时容忍 200 OK 响应，防止探针被误杀
...
* *  软件冷启动远程规则集未就绪说明：启动初期短暂穿透窗口。规则集在首次被求值时异步加载，加载完成前到达该规则位置的连接将因 Trie 为空而穿透至后续规则（MRS 下延迟极小）。
* *
* * 【behavior 行为模式性能原理】
...
* *  MRS 在此基础上进一步优化：以二进制序列化形式存储已构建的 Trie 树，加载时直接内存映射，跳过文本词法/语法分析及 Trie 构建，启动开销接近于零。
* *  yaml/text 格式需经"磁盘 IO → 反序列化 → Trie 构建"三阶段；MRS 仅需"磁盘 IO → 映射"。
* *
* * 【format 格式选择 — MRS 二进制预编译格式】
...
rule-providers:
  ...
  *   threat：        恶意特征（追踪器 / 隐私收集 / 恶意软件 / C2 命令与控制域名） → 安全防护
  ...
  *                     *    备用直链：https://github.com/peiyingyao/Rule-for-OCD/raw/refs/heads/master/rule/Clash/Advertising/Advertising_OCD_Domain.mrs
  ...
  anti-ad: * 🚫 综合拦截集（广告 / 跟踪 / 隐私 / 恶意软件 / 挖矿）
    ...
    *    覆盖范围：广告、跟踪器、隐私收集、恶意域名、挖矿。
    ...
    behavior: domain  * ℹ️ 行为：域名模式，MRS 直接内存映射已构建的后缀匹配树（Domain Trie）。
    format: mrs       * ℹ️ 格式：MRS 二进制格式，加载时直接映射，跳过 YAML/Text 反序列化与 Trie 构建，二进制解析开销极低。
    path: "./ruleset/providers/anti-ad.mrs" * 存储路径。规则集文件命名与规则集声明键值解耦
    interval: 86400   * ⏱️ 更新间隔：86400 秒 = 24 小时（内核运行时按该周期后台定时拉取；重启后亦检测缓存是否过期并重新拉取）。上游规则集变动较频繁，设较短缓存周期以跟进更新。
  ...
  * threat: * 🚫 威胁阻断集（范围未完全覆盖：广告 / 追踪器 / 隐私收集 / 挖矿，暂未覆盖恶意软件 / C2 命令与控制域名）
  *   type: http        * ℹ️ 类型：远程 HTTP 资源。
  *   behavior: domain  * ℹ️ 行为：域名模式，MRS 直接内存映射已构建的后缀匹配树（Domain Trie）。
  *   format: mrs       * ℹ️ 格式：MRS 二进制格式，加载时直接映射，跳过 YAML/Text 反序列化与 Trie 构建，二进制解析开销极低。
  ...
  *                     *    备用直链：https://github.com/peiyingyao/Rule-for-OCD/raw/refs/heads/master/rule/Clash/EasyPrivacy/EasyPrivacy_OCD_Domain.mrs
  ...
  non-cn: * 🛡️ 境外常用需代理域名集
    ...
    *    命中此规则集的流量送入锚点组 [🎛️ 节点选择]，经用户所选代理节点出站。
  ...
  hk-restricted: * 🇭🇰 港区受限网络服务专项规则集（含 OpenAI、Claude、Gemini 精准子域、TikTok 全球资源）
    type: http        * ℹ️ 类型：远程 HTTP 资源。
    behavior: domain  * ℹ️ 行为：域名模式，MRS 直接内存映射已构建的后缀匹配树（Domain Trie）。
    format: mrs       * ℹ️ 格式：MRS 二进制格式
...
rules:
  ...
  * ▌域名分流 — cn 规则集在前确保国内域名优先命中
  - RULE-SET,hk-restricted,🔀 港外优选     * 🇭🇰 专属港区受限网络服务高可用路由规则 → 走非港节点代理，置于分流集的顶层是避免被代理规则抢先命中而误走其他节点。
                                        * 应用场景示例：指定某些对香港 IP 极为敏感的 AI、流媒体等网络服务（含 OpenAI、Claude、Gemini、TikTok）的路由策略。
  - RULE-SET,amazon,DIRECT              * 🔓 个人扩展：亚马逊购物常用域名集 → 直连，基于 Domain Trie 极速命中。置于分流集的前列是为方便切换到代理时先于直连规则命中。
  - RULE-SET,cn,DIRECT                  * 🔓 国内主流域名 → 直连。精确域名匹配无需经过 GeoIP 推断，自然规避 CDN 误判归属地；小众域名盲区由归属地映射层（GEOIP,CN）补盲覆盖。
  - RULE-SET,non-cn,🎛️ 节点选择             * 🛡️ 境外代理域名 → 送入锚点组 [🎛️ 节点选择]，经用户所选代理节点出站。
  ...
  * - MATCH,🎛️ 节点选择      * 🛡️ 兜底代理：未命中流量送入锚点组 [🎛️ 节点选择]，适用于 Unknown → Proxy 立场：宁可错绕（走代理），不可错放（直连可能导致隐私泄露）。
...
* *   设为 true 时，内核启动自动将订阅内所有 outbound proxies（底层代理节点）及
* *   proxy sets（proxy-providers 代理提供者/订阅源 声明的节点集合）注入当前策略组
...
* *   ⚠️ 静默容错意味着失效无提示，首次启动无缓存且下载失败时，不会在日志中产生 error，需通过 CVR「规则集」界面主动检查条目数。
* *
...
* *    内核通过映射表逆向还原为域名后走域名规则匹配，不会直接命中 GEOIP 类规则。GEOIP 规则的 IP 比对仅对真实 IP 有效。
* *
* * REJECT（连接重置拦截）
...
* *   将域名解析为 IP 地址的分布式系统。
* *   传统 DNS（Do53）明文无加密，在污染环境下解析响应可能被篡改（MITM 或运营商注入伪造 IP），结果不可信；使用 DoH/DoT 的订阅配置因加密通道不受此影响。
* *
...
* *   进程之间传递数据的机制。此处特指 CVR 前端与 Mihomo 内核之间的 RPC（HTTP API）通信。核初始化阶段 Trie 构建为 CPU 密集型任务，可能造成调度延迟，
* *   导致 HTTP API goroutine 响应缓慢，CVR 前端报告超时或加载失败，但内核本身未崩溃——这是 RPC 层的通信超时，非进程崩溃。
* *
* * ──────────────────────────────────────────────────────────────────────────────────────
* * 【G】工程设计术语
* * ──────────────────────────────────────────────────────────────────────────────────────
...
* *   本配置中特指：include-all 未能继承任何节点时，[🎛️ 节点选择] 成为空组，
